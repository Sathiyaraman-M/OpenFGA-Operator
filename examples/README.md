...
- The Operator uses the `metadata.name` from `FgaStore` manifest to create a store in OpenFGA instance.
...
- Deleting the `FgaStore` resource will NOT delete the store in OpenFGA instance.
...
- Similar to `FgaStore` resource, deleting the `Model` resource will NOT delete the model in OpenFGA instance
...
- Unlike `FgaStore` and `FgaAuthorizationModel` resources, deleting the `FgaTupleSet` resource will delete the tuples from the OpenFGA instance.
